...
class StartExamCubit extends Cubit<StartExamState> {
  ...
  Future<void> checkAndStartExam({required int examId, String? accessCode}) async {
    ...
    try {
      ...
      if (responseData is Map<String, dynamic>) {
        serverMsg = responseData['message'] as String? ?? responseData['detail'] as String?;
      }
      ...
      if (code == 403) {
        if (serverMsg != null) {
          ...
          // Nếu thông báo từ BE chứa các từ khoá chỉ lỗi thời gian thi, Enroll hoặc quyền thi cứng
          if (normalizedMsg.contains('thời gian') ||
              normalizedMsg.contains('chưa') ||
              normalizedMsg.contains('quyền') ||
              normalizedMsg.contains('quá') ||
              normalizedMsg.contains('lượt')) {
            emit(StartExamFailure(message: serverMsg, statusCode: code));
            return;
          }
        }
        
        emit(StartExamCodeRequired(
          errorMessage: accessCode != null ? (serverMsg ?? 'Incorrect access code. Please try again.') : null,
        ));
        return;
      }

      final errorMsg = serverMsg ?? _getDioErrorMessage(e);
  ...
  void reset() => emit(const StartExamInitial());
}
