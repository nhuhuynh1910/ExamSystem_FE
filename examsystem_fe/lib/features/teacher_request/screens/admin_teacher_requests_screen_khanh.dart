...
class _AdminTeacherRequestsScreenKhanhState
  ...
  void _changeStatus(BuildContext context, String? status) {
    ...
    context.read<TeacherRequestCubitKhanh>().loadAdminRequests(status: status);
  }

  Future<void> _approve(BuildContext context, TeacherRequestModelKhanh request) async {
        ...
        return AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 9),
              Expanded(child: Text('Approve request')),
            ],
          ),
          content: Text(
              'Approve the request for "${request.subjectName}" from ${request.studentName}?'),
          actions: [
            TextButton(
    ...
    if (confirmed == true && mounted) {
      context.read<TeacherRequestCubitKhanh>().approveRequest(request.teacherRequestId);
    }
  }

  Future<void> _reject(BuildContext context, TeacherRequestModelKhanh request) async {
            ...
            return AlertDialog(
              ...
              title: const Row(
                children: [
                  ...
                  SizedBox(width: 9),
                  Expanded(child: Text('Reject request')),
                ],
              ),
              content: SizedBox(
                width: 420,
                child: Column(
                  ...
                  children: [
                    Text(
                      'Reject "${request.subjectName}" from ${request.studentName}.',
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    if (note.isEmpty) {
                      setDialogState(() => errorText = 'Rejection reason is required.');
                      return;
                    }
    ...
    if (adminNote != null && mounted) {
      context.read<TeacherRequestCubitKhanh>().rejectRequest(
  ...
  void _showDetails(BuildContext context, TeacherRequestModelKhanh request) {
                        ...
                        child: ListView(
                          ...
                          children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const Text('Teacher request details',
                                          style: TextStyle(
                            ...
                            const _DetailTitleKhanh(
                                icon: Icons.person_outline_rounded, title: 'Student information'),
                            const SizedBox(height: 10),
                            _DetailCardKhanh(
                              children: [
                                _DetailRowKhanh(label: 'Student name', value: request.studentName),
                                const Divider(height: 24),
                                _DetailRowKhanh(label: 'Email', value: request.studentEmail),
                              ],
                            ),
                            const SizedBox(height: 21),
                            const _DetailTitleKhanh(
                                icon: Icons.menu_book_outlined, title: 'Request information'),
                            const SizedBox(height: 10),
                            _DetailCardKhanh(
                              children: [
                                _DetailRowKhanh(label: 'Subject', value: request.subjectName),
                                const Divider(height: 24),
                                _DetailRowKhanh(
                                    label: 'Reason',
                                ...
                                const Divider(height: 24),
                                _DetailRowKhanh(
                                    label: 'Created date', value: _formatDate(request.createdAt)),
                              ],
                            ),
                            if (hasReviewer || request.reviewedAt != null) ...[
                              const SizedBox(height: 21),
                              const _DetailTitleKhanh(
                                  icon: Icons.fact_check_outlined, title: 'Review information'),
                              const SizedBox(height: 10),
                              _DetailCardKhanh(
                                children: [
                                  if (hasReviewer)
                                    _DetailRowKhanh(
                                        label: 'Reviewed by', value: request.reviewerName!),
                                  if (hasReviewer && request.reviewedAt != null)
                                    const Divider(height: 24),
                            ...
                            if (hasAdminNote) ...[
                              const SizedBox(height: 21),
                              const _DetailTitleKhanh(
                                  icon: Icons.info_outline_rounded, title: 'Admin note'),
                              const SizedBox(height: 10),
                              Container(
                                width: double.infinity,
                                padding: const EdgeInsets.all(14),
                            ...
                            if (hasCertificate) ...[
                              const SizedBox(height: 21),
                              const _DetailTitleKhanh(
                                  icon: Icons.attach_file_rounded, title: 'Certification'),
                              const SizedBox(height: 10),
                              Container(
                                width: double.infinity,
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () async {
                                        await Clipboard.setData(
                                            ClipboardData(text: request.certificationUrl!));
                                        if (mounted) {
                                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                              content: Text('Certification path copied.'),
                                              behavior: SnackBarBehavior.floating));
                                        }
                                      },
                                      icon: const Icon(Icons.copy_outlined, color: primaryColor),
                                    ),
                                  ],
                                ),
                              ),
            ...
            Builder(builder: (context) {
              return IconButton(
                ...
                onPressed: () => context
                    ...
                    .loadAdminRequests(status: _status),
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Padding(
                ...
                child: BlocConsumer<TeacherRequestCubitKhanh, TeacherRequestStateKhanh>(
                  listener: (context, state) {
                    if (state is TeacherRequestSuccessKhanh) {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: Text(state.message!), behavior: SnackBarBehavior.floating));
                    }
                    if (state is TeacherRequestErrorKhanh) {
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content: Text(state.error!), behavior: SnackBarBehavior.floating));
                    }
                  },
                    ...
                    return Column(
                      children: [
                        ...
                        _StatusFilterKhanh(
                          ...
                          onChanged: (status) => _changeStatus(context, status),
                        ),
                                        ...
                                        builder: (context, constraints) {
                                          ...
                                          if (columnCount == 1) {
                                            return ListView.builder(
                                              ...
                                              padding: const EdgeInsets.fromLTRB(12, 5, 12, 90),
                                              ...
                                              itemBuilder: (context, index) {
                                                ...
                                                return _TeacherRequestCardKhanh(
                                                  ...
                                                  onView: () => _showDetails(context, request),
...
class _AdminHeaderKhanh extends StatelessWidget {
  ...
  const _AdminHeaderKhanh({required this.searchController, required this.onSearchChanged});
...
class _StatusFilterKhanh extends StatelessWidget {
  ...
  const _StatusFilterKhanh({required this.selectedStatus, required this.onChanged});
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          _StatusChipKhanh(
              text: 'Pending',
              active: selectedStatus?.toLowerCase() == 'pending',
              activeColor: const Color(0xffff8a00),
              onTap: () => onChanged('Pending')),
          _StatusChipKhanh(
              text: 'Approved',
              active: selectedStatus?.toLowerCase() == 'approved',
              activeColor: const Color(0xff25a84c),
              onTap: () => onChanged('Approved')),
          _StatusChipKhanh(
              text: 'Rejected',
              active: selectedStatus?.toLowerCase() == 'rejected',
...
class _StatusChipKhanh extends StatelessWidget {
  ...
  const _StatusChipKhanh(
      {required this.text, required this.active, required this.activeColor, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Material(
        color: active ? activeColor : const Color(0xffedf1f7),
...
class _TeacherRequestCardKhanh extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String firstLetter =
        request.studentName.trim().isNotEmpty ? request.studentName.trim()[0].toUpperCase() : '?';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(color: Color(0x0d000000), blurRadius: 9, offset: Offset(0, 3))
        ],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Text('#REQ-${request.teacherRequestId.toString().padLeft(4, '0')}',
                  style: const TextStyle(
                      color: Color(0xff637da5), fontSize: 9, fontWeight: FontWeight.w900)),
              const Spacer(),
              _RequestStatusBadgeKhanh(
                  text: request.status, color: statusColor, background: statusBackground),
            ],
          ),
          ...
          Text(request.subjectName,
              ...
              style: const TextStyle(
                  color: textColor, fontSize: 15, height: 1.3, fontWeight: FontWeight.w900)),
          const SizedBox(height: 9),
          Row(
            children: [
              Container(
                ...
                child: Text(firstLetter,
                    style: const TextStyle(
                        color: primaryColor, fontSize: 11, fontWeight: FontWeight.w900)),
              ),
          ...
          const Text('REASON',
              style: TextStyle(
                  color: Colors.blueGrey, fontSize: 9, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
          ...
          Text(
              ...
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(
                  color: Colors.blueGrey, fontSize: 11, height: 1.4, fontStyle: FontStyle.italic)),
          const SizedBox(height: 11),
          const Divider(height: 1, color: Color(0xffffe5dc)),
          const SizedBox(height: 9),
          Row(
            children: [
              Expanded(
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Text(text,
              style: TextStyle(color: foregroundColor, fontSize: 9, fontWeight: FontWeight.w900)),
        ),
      ),
    );
  }
...
class _RequestStatusBadgeKhanh extends StatelessWidget {
  ...
  const _RequestStatusBadgeKhanh({required this.text, required this.color, required this.background});
...
class _DetailTitleKhanh extends StatelessWidget {
  ...
  const _DetailTitleKhanh({required this.icon, required this.title});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon, size: 19, color: primaryColor),
        const SizedBox(width: 7),
        Text(title,
            style: const TextStyle(color: textColor, fontSize: 15, fontWeight: FontWeight.w900)),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(label,
            style:
                const TextStyle(color: Colors.blueGrey, fontSize: 11, fontWeight: FontWeight.w600)),
        const SizedBox(height: 5),
        Text(value,
            style: const TextStyle(
                color: textColor, fontSize: 13, height: 1.45, fontWeight: FontWeight.w700)),
      ],
    );
  }
}
...
class _EmptyStateKhanh extends StatelessWidget {
  ...
  const _EmptyStateKhanh(
      {required this.isSearching, required this.selectedStatus, required this.onRefresh});
  ...
  Widget build(BuildContext context) {
    ...
    if (isSearching) {
      ...
      message = 'No ${selectedStatus!.toLowerCase()} teacher requests found.';
    } else {
        ...
        children: [
          Center(
            child: Container(
              ...
              height: 82,
              decoration: const BoxDecoration(color: Color(0xffffebe4), shape: BoxShape.circle),
              child: Icon(isSearching ? Icons.search_off_rounded : Icons.inbox_outlined,
                  color: primaryColor, size: 39),
            ),
          ),
